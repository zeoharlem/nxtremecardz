...
class CartController extends BaseController {
    ...
    public function initialize(){
        ...
        $this->tag->appendTitle("Cart View");
        ...
        if($this->session->has("regAuth")) {
            try {
                $transaction = $this->paystack->transaction->initialize([
                    "amount" => ($this->getProductTotal()) * 100,
    ...
    public function checkoutAction(){
            ...
            if($this->session->has("auth")){
                ...
                if($billingRegister->create($this->request->getPost())){
                    ...
                    $this->response->setJsonContent(
                        [
                            "status"    => "ERROR",
                            "data"      => implode(",",$billingRegister->getMessages())
                        ]
                    )->send();
                }
            }
                ...
                try {
                    $billingRegister = new BillingRegister();
                    $billingRegister->setTransaction($transaction);
                    if (!$billingRegister->create($this->request->getPost())) {
                        $getMessages = implode(",", $billingRegister->getMessages());
                        $transaction->rollback("Error: " . $getMessages);
                    }
                    $UsersBilling = new Users();
                    $UsersBilling->setTransaction($transaction);
                    $arrayPostTask = [
                    ...
                    if ($UsersBilling->create($arrayPostTask) == false){
                        $getMessages = implode(",", $UsersBilling->getMessages());
                        $transaction->rollback("Error: " . $getMessages);
                    }
                    $transaction->commit();
        ...
        if($this->session->has("cart_item")){
            foreach($this->session->get("cart_item") as $keyRow => $valueRow){
                $totalAmount[]  = $valueRow['quantity'] * $valueRow['price'];
            }
        }
        $this->view->setVars([
            "totalAmount"   => array_sum($totalAmount),
            "totalAmountFt" => number_format(array_sum($totalAmount), 2)
        ]);
    ...
    public function paycheckedAction(){
        ...
        if(!$this->session->has("regAuth")){
            return $this->response->redirect("cart/checkout?task=fill_form");
        }
        ...
        if ('success' === $tranx->data->status) {
            ...
            $orderRowPay    = [
                ...
                "items"         => json_encode($this->session->get("cart_item")),
            ...
            if($order->create($orderRowPay) != false){
                ...
                return $this->response->redirect("index?task=failed");
            }
        }
    }
    ...
    public function successAction(){
        if(!$this->request->hasQuery("path")){
            return $this->response->redirect("index?task=SUCCESS");
        }
