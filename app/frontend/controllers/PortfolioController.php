...
class PortfolioController extends BaseController {
    ...
    public function indexAction(){
        $this->view->setVars(["catproducts" => $this->category()]);
    ...
    private function category(){
        ...
        foreach($category as $keyRow => $valueRow){
            $getCartPortfolio   = $valueRow->getCatPortfolio();
            foreach($getCartPortfolio as $keyCart => $valueCart){
                $getImages = $valueCart->getAlbums()->getAlbumsImages();
                if(!empty($getImages)){
                    $counter    = 0;
                    foreach($getImages as $keyGetImages => $valueGetImages){
                        if($counter < 2)
                            $stackRow[$valueRow->name][$valueRow->category_id][$counter] = $valueGetImages->toArray();
                        $counter++;
                    }
                }
            }
        }
        return !empty($stackRow) ? $stackRow : array();
    }
}